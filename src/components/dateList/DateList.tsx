...
function DateList() {
  ...
  const [selected, setSelected] = React.useState<string>(moment().format("DD MM YYYY"));
  ...
  const date = () => {
    ...
    for(var i=15; i>0; i--) {
      result.push({date: moment().subtract(i, 'days').format("ddd DD"), id: moment().subtract(i, 'days').format("DD MM YYYY")})
    }
    for(i=0; i<=15; i++) {
      result.push({date:  moment().add(i, 'days').format("ddd DD"), id: moment().add(i, 'days').format("DD MM YYYY")})
    }
    return result;
  }
