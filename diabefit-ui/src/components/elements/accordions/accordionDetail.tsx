...
const CustomizedAccordions: React.FC<Props> = ({ elementsProps, changedData }) => {
          ...
          elementsProps.map((e) => {
            return (
              <ListItem style={{padding: '2px 4px'}} key={e.header}>
                ...
                <IconButton edge="end" color="inherit" aria-label="close" size="small" onClick={()=> removeMealElement(e.dayID, e.dayIndex, e.id)}>
                  <CloseIcon />
                </IconButton>
              </ListItem>
            );
