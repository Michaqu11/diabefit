...
const formatServingToOneHundredGrams = (serving: any) => {
  const multiplier =
    Number(HUNDRED_GRAMS) / Number(serving.metric_serving_amount ?? 1);
  return {
    metric_serving_amount: Number(
      (serving.metric_serving_amount * multiplier).toFixed(1),
    ),
...
export const setupServingsData = (servings: any) => {
  const hundredGramsSarvingExist = servings.filter(
    (s: any) => s.metric_serving_amount === HUNDRED_GRAMS,
  );
  return formatServingToOneHundredGrams(
    hundredGramsSarvingExist.length ? hundredGramsSarvingExist[0] : servings[0],
  );
};

export const formatMealToCustomGrams = (
  meal: IMealElement,
  grams: number,
): IMealElement => {
